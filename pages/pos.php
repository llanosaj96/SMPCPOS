...
if(filter_input(INPUT_POST, 'addpos')){
    ...
    $sqls = mysqli_query($db, "SELECT * FROM product WHERE PRODUCT_ID = '$check_product_stock' AND ENDED !='YES' ");
    while ($rows = mysqli_fetch_assoc($sqls)) {
      $stock_on_hand = $rows['ON_HAND'];
      ...
      if ($stock_on_hand < $customer_qty) {
        ...
        $sql02 = mysqli_query($db, "UPDATE product SET ON_HAND='$new_on_hand', ENDED='$ended', CHECK_IT='1' WHERE PRODUCT_ID='$check_product_stock'  ");
        $new_sold = '';
...
// PARA SA DELIVERY NGA TRANSACTION KUNG MAG ADD NAKA!!!


if(filter_input(INPUT_POST, 'addpos_delivery')){
    $check_product_stock = @$_POST['product_id'];
    $customer_qty = @$_POST['quantity'];
    $ended = '';
    ...
    while ($rows = mysqli_fetch_assoc($sqls)) {
      ...
      $del_ids = $rows['DELIVERY_ID'];
      $new_on_hand = $stock_on_hand - $customer_qty;
      if ($stock_on_hand < $customer_qty) {
        echo "<script> alert('Product out of stock!'); </script>";
      }else{

        // SA DELIVERY NGA DATABASE 

        $sqld1 = mysqli_query($db, "SELECT * FROM delivery WHERE rec_no='$del_ids' ");
        while ($rowd1 = mysqli_fetch_assoc($sqld1)) {
          $del_sold = $rowd1['SALES'];
          $del_sold1 = $del_sold + $customer_qty;
          $sqld2 = mysqli_query($db, "UPDATE delivery SET SALES='$del_sold1' WHERE rec_no='$del_ids' ");
        ...
        $sql02 = mysqli_query($db, "UPDATE product SET DELIVERY_QTY='$new_on_hand', DELIVERY_ENDED='$ended', CHECK_IT='2' WHERE PRODUCT_ID='$check_product_stock'  ");
        $new_sold = '';
        $sql03 = mysqli_query($db, " SELECT * FROM product WHERE PRODUCT_ID='$check_product_stock' ");
        while ($row03 = mysqli_fetch_assoc($sql03)) {
          $current_sold = $row03['SOLD'];
    ...
    foreach($_SESSION['pointofsale'] as $key => $product){
        if ($product['id'] == filter_input(INPUT_GET, 'id')){
            ...
            while ($row002 = mysqli_fetch_assoc($sql002)) {
              $stock_onh = $row002['ON_HAND'];
              $del_id = $row002['DELIVERY_ID'];
              $ref_id = $row002['REF_ID'];
              $check_delivery_qty = $row002['DELIVERY_QTY'];
              $check_delivery_ended = $row002['DELIVERY_ENDED'];
              $end = $row002['ENDED'];
              $CHECKS = $row002['CHECK_IT'];


              if ($check_delivery_qty == 0 && $check_delivery_ended == 'YES' && $CHECKS == 2) {
                // KUNG E DIRITSO UG PURCHASE TANAN SA DELIVERIHAN !!!!

                $new_onh1 = $check_delivery_qty + $p_qty;
                $sql004 = mysqli_query($db, "UPDATE product SET DELIVERY_QTY='$new_onh1', DELIVERY_ENDED='NO', CHECK_IT='0' WHERE PRODUCT_ID='$p_id' ");
                $sql1111 = mysqli_query($db, "UPDATE delivery SET STATUS = 'OUT FOR DELIVERY' WHERE rec_no='$del_id' ");

                $new_sold1 = '';
                $sql04 = mysqli_query($db, " SELECT * FROM product WHERE PRODUCT_ID='$p_id' ");
                while ($row04 = mysqli_fetch_assoc($sql04)) {

                  $current_sold1 = $row04['SOLD'];
                  $new_sold1 = $current_sold1 - $p_qty;   
                  $sql05 = mysqli_query($db, "UPDATE product SET SOLD='$new_sold1' WHERE PRODUCT_ID='$p_id' ");

                  $sqlZ = mysqli_query($db, "SELECT * FROM delivery WHERE rec_no='$del_id' ");
                  while ($rowZ = mysqli_fetch_assoc($sqlZ)) {

                    $soldDeliver = $rowZ['SALES'];
                    $sold_new_delivered = $soldDeliver - $p_qty;
                    if ($soldDeliver == 0) {
                       $sqlx = mysqli_query($db, "UPDATE delivery SET SALES='$sold_new_delivered', STATUS='COMPLETED' WHERE rec_no='$del_id' ");
                    }else{
                       $sqlxS = mysqli_query($db, "UPDATE delivery SET SALES='$sold_new_delivered', STATUS='OUT FOR DELIVERY' WHERE rec_no='$del_id' ");
                    }
              ...
              }else if($check_delivery_qty != 0 && $check_delivery_ended == 'NO' && $CHECKS == 2 ){
                // KUNG TINGUB ANG PAG PURCHASE SA DELIVERYHAN
                
                $new_onh2 = $check_delivery_qty + $p_qty;
                $sql2 = mysqli_query($db, "UPDATE product SET DELIVERY_QTY='$new_onh2', CHECK_IT='0' WHERE PRODUCT_ID='$p_id' ");

                $new_sold2 = '';
                $sql222 = mysqli_query($db, " SELECT * FROM product WHERE PRODUCT_ID='$p_id' ");
                while ($row222 = mysqli_fetch_assoc($sql222)) {

                  $current_sold22 = $row222['SOLD'];
                  $new_sold2 = $current_sold22 - $p_qty;
                  $sql33 = mysqli_query($db, "UPDATE product SET SOLD='$new_sold2' WHERE PRODUCT_ID='$p_id' ");

                  $sqlZz = mysqli_query($db, "SELECT * FROM delivery WHERE rec_no='$del_id' ");
                  while ($rowZz = mysqli_fetch_assoc($sqlZz)) {

                    $soldDeliver1 = $rowZz['SALES'];

                    $sold_new_delivered1 = $soldDeliver1 - $p_qty;
                    
                    $sqlxX = mysqli_query($db, "UPDATE delivery SET SALES='$sold_new_delivered1', STATUS='OUT FOR DELIVERY' WHERE rec_no='$del_id' ");
                ...
                if ($stock_onh == 0) {
                  $sqlll = mysqli_query($db, "UPDATE product SET ENDED='NO' WHERE PRODUCT_ID='$p_id' ");
                }
                  $new_onh = $stock_onh + $p_qty;
                    $sql003 = mysqli_query($db, "UPDATE product SET ON_HAND='$new_onh' WHERE PRODUCT_ID='$p_id' ");
                    ...
                    while ($row03 = mysqli_fetch_assoc($sql03)) {
                      ...
                      $sql04 = mysqli_query($db, "UPDATE product SET SOLD='$new_sold', CHECK_IT='0' WHERE PRODUCT_ID='$p_id' ");
                      unset($_SESSION['pointofsale'][$key]);
                    }
