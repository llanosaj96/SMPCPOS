...
<div id="header" style="display: none;">
  ...
  <b>
  <label style="font-size: 18px; font-family: calibri;">Sales Report: <?php echo $month1; echo " "; echo $day; echo ", "; echo $year; ?> </label>
</b>
  </center>
  <br/>
</div>
 ...
 <thead>
  ...
  <tbody>
    <?php
      ...
      $sqlA = mysqli_query($db, "SELECT * FROM transaction T JOIN customer C ON T.`CUST_ID`=C.`CUST_ID` JOIN transaction_details TD ON T.`TRANS_D_ID`=TD.`TRANS_D_ID` WHERE DAY(DATE)='$day' AND MONTH(DATE)='$month' AND YEAR(DATE)='$year' ORDER BY OR_NUMBER asc ");
      while ($rows = mysqli_fetch_assoc($sqlA)) {
        ...
        $orn = $rows['OR_NUMBER'];
          ...
          $sqlB = mysqli_query($db, "SELECT SUM(QTY*PRICE) as TTL FROM transaction_details TD JOIN transaction TL ON TD.`TRANS_D_ID`=TL.`TRANS_D_ID` WHERE DAY(DATE)='$day' AND MONTH(DATE)='$month' AND YEAR(DATE)='$year' ");
