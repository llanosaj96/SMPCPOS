...
<label style="font-size: 18px; font-family: calibri;">Product Inventory Date: <span> <?php echo $month1; echo " "; echo $year; ?> </span> </label><br/>
<label style="font-size: 18px; font-family: calibri;">Category: <?php echo $cname; ?> </label>
</b> <br/><br/>
</div>
...
<?php
    ...
    $query = "SELECT * FROM product p  JOIN category c on p.CATEGORY_ID=c.CATEGORY_ID WHERE p.CATEGORY_ID='$category' AND p.DATE_STOCK_IN_MONTH='$month' AND p.DATE_STOCK_IN_YEAR='$year' GROUP BY PRODUCT_CODE";
        $result = mysqli_query($db, $query) or die (mysqli_error($db));
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $pid = $row['PRODUCT_ID'];
                $PC = $row['PRODUCT_CODE'];
                $dsm = $row['DATE_STOCK_IN_MONTH'];
                $dsy = $row['DATE_STOCK_IN_YEAR'];
                $sqlll = mysqli_query($db, "SELECT SUM(IN_STOCK) as ttl_inn FROM inventory_update WHERE PRODUCT_CODE='$PC' AND MONTH(UPDATED_DATE) = '$dsm' AND YEAR(UPDATED_DATE) = '$dsy' ");
                $rowssql = mysqli_fetch_assoc($sqlll);
                $overall_in = $rowssql['ttl_inn'];
                $overall_in2 = $overall_in + $beg;
                $overall_in3 = $overall_in2 - $sol;
                $ttl_beg = $r_price * $beg;
                $ttl_beg1 = number_format($ttl_beg, 2);
                $ttl_stock_price = $qty * $r_price;
                $total01 = number_format($ttl_stock_price, 2);
                $ttl_amount_sold = $sol * $r_price;
                $total02 = number_format($ttl_amount_sold, 2);
                $cash = $row['CASH'];
                $credit = $row['CREDIT'];
                $ttl_on_hand = $overall_in2 - $sol;                
                $new_total = ((double)$ttl_on_hand * (double)$r_price);
                ...
                $in_ttl = $r_price * $sol;
                $sol_ttl = $price * $sol;
                $total_over_sales = ((double)$sol_ttl - (double)$in_ttl);
                ...
                echo "<td>$r_price2</td>";
                echo "<td>$ttl_beg1</td>";
                echo "<td>$overall_in</td>";
                echo "<td>$r_price2</td>";
                echo "<td>  $total01</td>";
                echo "<td> $sol ($cash)-cash ($credit)-credit </td>";
                // echo "<td> $prr </td>";
                echo "<td>  $total02</td>";
                echo "<td>$overall_in3</td>"; // instead of $ttl_on_hand
                 echo "<td>$ret_cases</td>";
                echo "<td>  $total03</td>";
                echo "<td>  $price8</td>";
